...
static Function *to_function(jl_lambda_info_t *li, jl_cyclectx_t *cyclectx)
{
    ...
    JL_TRY {
        ...
        #endif
        f = (Function*)definitions.functionObject;
        specf = (Function*)definitions.specFunctionObject;
...
static Value *emit_lambda_closure(jl_value_t *expr, jl_codectx_t *ctx)
{
    ...
    Value *gcroot = emit_local_slot(ctx);
    builder.CreateStore(env_tuple, gcroot);
#ifdef LLVM37
    ...
    Value *result = builder.CreateCall3(prepare_call(jlclosure_func),
                                        ...
                                        env_tuple, literal_pointer_val(expr));
#endif
    builder.CreateCall(prepare_call(gckill_func), gcroot);
    return result;
}
                                        ...
                                        jl_codectx_t *ctx)
{
    ...
    return mark_julia_type(emit_jlcall(theFptr, theF, &args[1], nargs, ctx), true, jl_any_type, ctx); // (typ will be patched up by caller)
}
...
static jl_cgval_t emit_call(jl_value_t **args, size_t arglen, jl_codectx_t *ctx, jl_value_t *expr)
{
    ...
    if (definitely_not_function) {
        ...
        if (theFptr == NULL) {
            ...
            if (theFptr == NULL) {
                Value *theFunc = get_gcrooted(emit_expr(args[0], ctx), ctx);
        ...
        result = mark_julia_type(ph, true, jl_any_type, ctx);
    }
...
static void init_julia_llvm_env(Module *m)
{
    ...
    add_named_global(jltls_states_func, NULL, /*dllimport*/false);
#else
